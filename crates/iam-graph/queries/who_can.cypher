...
//   (3) entity with Action:'*' (full-admin). Each arm excludes entities that have an
//   explicit Deny on $action or '*' in their own policies. Deny scope is approximate:
//   only exact-action and Action:'*' Denies are checked; wildcard Denies (e.g. s3:Delete*)
//   and group-inherited Denies for a user's own policies are not evaluated (see limitations.md).
