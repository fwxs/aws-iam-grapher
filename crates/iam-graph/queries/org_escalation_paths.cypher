...
//   stitch pass) to find entities that can reach a configured risky-action group by
//   assuming roles across account boundaries. Only transitive paths (1..N hops) are
//   returned; run privilege_escalation_paths per-account for the zero-hop (direct) case.
...
//   risky-action group — filters which Permission nodes are pulled back. AND/OR group
//   semantics are evaluated in Rust afterward, post-Deny-subtraction.
