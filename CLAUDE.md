...
- `privilege_escalation_paths()` — entities holding permissions from a configured risky-action group (`config/risky-actions.yaml`, AND-within-group/OR-across-group; `--risky-actions <path>` or `~/.aws-iam-grapher/config/risky-actions.yaml`, fatal if neither resolves — see `iam-graph::risky_actions` and the `config check` subcommand)
